...
# raw J1708 messages, J1587Driver abstracts away some details. Firts, import J1587Driver the same way
# that we imported J1708Driver
from hv_networks import J1587Driver
...
def decode_vehicle_distance(byte_string):
    ...
    data_len = byte_string[2]
    total_distance_raw = struct.unpack('<L', byte_string[3:3+data_len])[0]

    total_distance_scaled = total_distance_raw * 0.1

    return total_distance_scaled
...
driver = J1587Driver.J1587Driver(0xac)
...
for pid in pids:
    ...
    elif pid == 237:#Insert the PID for VIN here
        formatted_output = decode_vin(msg)
    elif pid == 244:#Insert the PID for Trip Distance here
        formatted_output = decode_trip_distance(msg)
    elif pid == 245:#Insert the PID for Vehicle Distance here
